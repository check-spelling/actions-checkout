    ...
    try {
      ...
      if (clean) {
        ...
        if (!(await git.tryClean())) {
          core.debug(
            `The clean command failed. This might be caused by: 1) path too long, 2) permission issue, 3) file in use, 4) object is a volume. For further investigation, manually run 'git clean -ffdx' on the directory '${repositoryPath}'.`
          )
          remove = true
  ...
  if (remove) {
    ...
    for (const file of await fs.promises.readdir(repositoryPath)) {
      ...
      try {
        ...
        await exec.exec(`sudo chown -R ${whoami} ${filePath}`)
        await io.rmRF(filePath)
      }
    }
  }
